...
CHECK_INFO_CUSTOMER() {
  ...
  then
    ...
    # ask time service 
      ...
      echo -e "\nI have put you down for a $TYPE_SERVICE at $SERVICE_TIME, $CUSTOMER_NAME."
      # display services what customer selected
      echo -e "\nHere are services you selected:\n"
      DISPLAY_SERVICES=$($PSQL "select service_id, services.name from services full join appointments using(service_id) full join customers using(customer_id) where phone='$CUSTOMER_PHONE'")
      display=$(echo "$DISPLAY_SERVICES" | sed 's/|/) /')
      echo "$display" | while read info
      do
        echo $info
      done
    else
      #send to main menu
      MAIN_MENU "You selected it. Please selecting the other."
    fi
  fi
}
